...
def test_calculate_p_value_1_process():
    ...
    assert np.array_equal(p_values, ['NOTEST', 'NOTEST', 'NOTEST', 'NOTEST', 0.0, 'NOTEST', 'NOTEST', 0.0, 'NOTEST', 0.0])

def test_calculate_p_value_5_processes():
    cwd = os.getcwd()
    cwd = cwd.split('RPx')[0]

    detector = src.detector(cwd + "RPx/datasets/young_values.txt", 1, 2, filter_detectable = True, filter_zero = True, num_permutations = 10, num_procs = 5)

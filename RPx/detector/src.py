...
class detector(object):
    ...
    def __repr__(self):
        return "<File {}, Period: {}, n_Cycles: {}, num_procs: {}, filter_detectable: {}, filter_zero: {}>".format(
            self.config["FILE_NAME"],
            self.config["PERIOD"],
            self.config["N_CYCLES"],
            self.config["NUM_PROCS"],
            self.config["FILTER_DETECTABLE"],
            self.config["FILTER_ZERO"],
        )
    ...
    def read_file(self, delim=None):
        if delim not in (None, "tab"):
            raise KeyError(
                "delimiter not recognized, please use 'tab' if tab delimited wanted\n default: comma separated"
            )
        elif delim == "tab":
            delim = "\t"
        else:
        ...
        signals["detectable"] = signals.iloc[
            :, 1 : (self.config["LEN_SIGNALS"] + 1)
        ].apply(self._detectable, axis=1)
        signals["zero"] = signals.iloc[:, 1 : (self.config["LEN_SIGNALS"] + 1)].apply(
            self._contains_zero, axis=1
        )
    ...
    def compute_rpx(self, signal):
        ...
        RPx = np.log2(
            (
                PSD_mod[self.config["PERIOD"] * self.config["N_CYCLES"]]
                + self.config["EPS"]
            )
            / (
                sum(PSD_mod[0 : int(len(PSD_mod) / 2 + 1)])
                - PSD_mod[self.config["PERIOD"] * self.config["N_CYCLES"]]
                + self.config["EPS"]
            )
        )
        ...
        if RPx >= self.config["MIN_RP24"]:
            ...
            for i in range(self.config["NUM_PROCS"]):
                process = multiprocessing.Process(
                    target=self._compute_permutations, args=(signal, queue,)
                )
                jobs.append(process)
                process.start()
    ...
    def detect(self, signals):
        signals["RPx"] = signals.iloc[:, 1 : (self.config["LEN_SIGNALS"] + 1)].apply(
            self.compute_rpx, axis=1
        )

        indexes = list(range(1, self.config["LEN_SIGNALS"] + 1)) + [
            len(signals.columns.tolist()) - 1
        ]  # separated for clarity - just the signal indexes and the index of the RPx
        signals["p_values"] = signals.iloc[:, indexes].apply(
            self.compute_p_value, axis=1
        )

        signals["q_values"] = self.compute_q_values(signals["p_values"])

        signals["phase"] = signals.iloc[:, 1 : (self.config["LEN_SIGNALS"] + 1)].apply(
            self.compute_phase, axis=1
        )
        signals["mean"] = signals.iloc[:, 1 : (self.config["LEN_SIGNALS"] + 1)].apply(
            np.mean, axis=1
        )
        return signals
